...
export async function createCustomFormType(name: string): Promise<{
  id: string;
}> {
  const id = uuidv4();
  const uri = `http://data.lblod.info/id/lmb/forms/custom-types/${id}`;
  ...
  await update(`
    ...
    INSERT DATA {
      ${sparqlEscapeUri(uri)} a ext:CustomFormType ;
        mu:uuid ${sparqlEscapeString(id)} ;
...
export async function fetchCustomFormTypeList() {
...
export async function createEmptyCustomForm(
  name: string,
  typeId: string,
): Promise<{
  id: string;
  ...
  const safeName = name.replace(/[^a-zA-Z0-9]/g, '');
  const id = uuidv4();
  const uri = `http://data.lblod.info/id/lmb/forms/${safeName}/${id}`;
  ...
  await update(`
    ...
    INSERT DATA {
        ${sparqlEscapeUri(uri)} a form:Form, ext:GeneratedForm ;
            ext:isCustomForm true ;
            mu:uuid ${sparqlEscapeString(id)} ;
            form:targetLabel ${sparqlEscapeString(name)} ;
            form:targetType ${sparqlEscapeUri(typeId)} .
    }
  `);
  return { id };
}

export async function fetchCustomFormInstanceList() {
  const queryResult = await query(`
    PREFIX form: <http://lblod.data.gift/vocabularies/forms/>
    PREFIX mu: <http://mu.semte.ch/vocabularies/core/>
    PREFIX ext: <http://mu.semte.ch/vocabularies/ext/>
    ...
    SELECT ?id ?name ?type
    WHERE {
      ?form a form:Form, ext:GeneratedForm ;
        ext:isCustomForm true ;
        mu:uuid ?id ;
        form:targetLabel ?name ;
